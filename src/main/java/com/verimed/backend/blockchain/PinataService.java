...
public class PinataService {
    ...
    public String uploadToPinata(Map<String, Object> jsonData) {
        ...
        if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
            Object ipfsHash = response.getBody().get("IpfsHash");
            if (ipfsHash != null) {
                System.out.println("JSON subido a Pinata. CID: " + ipfsHash.toString());
                return ipfsHash.toString();
            }
            throw new RuntimeException("Respuesta de Pinata sin IpfsHash");
        } else {
